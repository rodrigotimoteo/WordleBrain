...
/// This is used both by the CLI (which caches to disk) and by WASM (which embeds the cache).
pub fn compute_first_turn_cache(all_words: &[String]) -> HashMap<String, f64> {
...
/// Accepts a pre-computed first-turn cache for efficiency.
pub struct EntropySolver {
    pub first_turn_cache: HashMap<String, f64>,
}
        ...
        if history.is_empty() {
            ...
            for guess in all_words {
                let e = self
                    ...
                    .get(guess)
                    .copied()
                    .unwrap_or_else(|| compute_entropy_raw(guess, remaining));
                if e > best_entropy {
                    best_entropy = e;
